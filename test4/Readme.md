...
<!--注：删除表的同时回一起删除主外键、触发器、程序包-->

##### 4.创建表DEPARTMENTS和EMPLOYEES
...
##### 6.为EMPLOYEES表的DEPARTMENT_ID字段添加外键约束EMPLOYEES_FK1，引用DEPARTMENTS部门表的DEPARTMENT_ID字段
...
##### 7.为EMPLOYEES表的MANAGER_ID字段添加外键约束EMPLOYEES_FK2，引用EMPLOYEES部门表的EMPLOYEE_ID字段，在删除EMPLOYEES表中的数据时，引用表EMPLOYEES的EMPLOYEE_ID字段置为null
...
ON DELETE SET NULL ENABLE;ALTER TABLE EMPLOYEES
ADD CONSTRAINT EMPLOYEES_FK1 FOREIGN KEY
(
  DEPARTMENT_ID
)
...
##### 8.为EMPLOYEES表的SALARY字段添加检查约束EMPLOYEES_CHK1：工资大于0
...
##### 9.为EMPLOYEES表添加检查约束EMPLOYEES_CHK2：EMPLOYEE_ID与MANAGER_ID不相等
...
##### 10.为EMPLOYEES表添加检查约束EMPLOYEES_EMPLOYEE_MANAGER_ID：EMPLOYEE_ID与MANAGER_ID不相等
...
##### 11.为EMPLOYEES表添加检查约束EMPLOYEES_SALARY：SALARY>0
...
##### 12.创建表PRODUCTS，并且添加检查约束PRODUCTS_CHK1，限制产品类型在'耗材', '手机', '电脑'中
...
##### 13.创建全局临时表ORDER_ID_TEMP，只对当前会话有效
...
##### 14.创建ORDERS表，根据ORDER_DATE字段划分三个分区
...
##### 16.创建索引ORDERS_INDEX_CUSTOMER_NAME，根据CUSTOMER_NAME字段升序索引。
...
##### 17.创建索引ORDERS_INDEX_CUSTOMER_NAME，根据CUSTOMER_NAME字段升序索引。
...
##### 18.为ORDERS表ORDER_ID字段添加主键约束ORDERS_PK。
...
##### 19.为ORDERS表EMPLOYEE_ID字段添加外键约束ORDERS_FK1。引用EMPLOYEES表的EMPLOYEE_ID字段
...
##### 21.为ORDER_DETAILS表的ID字段创建唯一索引ORDER_DETAILS_PK
...
##### 22.为ORDER_DETAILS表的ID字段创建主键约束ORDER_DETAILS_PK
...
##### 23.为ORDER_DETAILS表的ORDER_ID字段创建全局索引ORDER_DETAILS_ORDER_ID
...
##### 24.为ORDER_DETAILS添加检查约束：Product_Num>0，产品数量大于零
...
##### 26.创建两个序列号SEQ_ORDER_ID、SEQ_ORDER_DETAILS_ID
